...
import { careersSchema } from '../data/seoSchemas';
...
export const CareersPage: React.FC = () => {
  ...
  const [applicantData, setApplicantData] = useState({
    fullName: '', email: '', phone: '', linkedinUrl: '', coverNote: ''
  });
    ...
    <div className="min-h-screen">
        ...
        <div className="max-w-4xl mx-auto">
          ...
          {!loading && !fetchError && jobs.length === 0 && (
            <div className="border border-base-300 dark:border-white/10 rounded-2xl p-10 text-center">
              ...
              <button
                type="button"
                onClick={() => { setApplyModalRole('general'); setSubmitted(false); }}
          ...
          {!loading && !fetchError && filteredJobs.length > 0 && (
            <div className="space-y-4">
              {filteredJobs.map(r => (
                <div key={r._id} className="border border-base-300 dark:border-white/10 bg-base-100 dark:bg-navy-light rounded-2xl overflow-hidden shadow-sm">
                  <div className="p-6 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                    <div>
                      <h3 className="font-bold text-lg text-base-content dark:text-white mb-1">{r.title}</h3>
                      <div className="flex flex-wrap gap-3 text-xs text-base-content/60 dark:text-white/60">
                        <span>{r.department}</span>
                        <span>·</span>
                        <span>{r.location}</span>
                        <span>·</span>
                        <span>{formatType(r.type)}</span>
                      </div>
                    </div>
                    <div className="flex gap-2.5 w-full sm:w-auto">
                      <button
                        ...
                        className="flex-1 sm:flex-none px-4 py-2 border border-base-300 dark:border-white/20 hover:bg-base-200 dark:hover:bg-white/5 rounded text-xs font-semibold text-base-content dark:text-white transition-all"
                      >
                        {expandedRole === r._id ? 'Hide Details' : 'View Role'}
                      </button>
                      <button
                        type="button"
                        onClick={() => { setApplyModalRole(r._id); setSubmitted(false); }}
                  ...
                  {expandedRole === r._id && (
                    <div className="p-6 pt-0 border-t border-base-200 dark:border-white/10 mt-2 space-y-4 text-xs sm:text-sm text-base-content/80 dark:text-white/80">
      ...
      {applyModalRole && (
        <div className="fixed inset-0 z-50 bg-navy/80 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-base-100 dark:bg-navy-light border border-base-300 dark:border-white/10 rounded-2xl w-full max-w-lg p-6 sm:p-8 shadow-2xl relative">
            <button
              type="button"
              onClick={() => setApplyModalRole(null)}
              className="absolute top-4 right-4 text-base-content/60 dark:text-white/60 hover:text-base-content dark:hover:text-white text-lg"
            >
              ✕
            </button>
            ...
            {submitted ? (
              ...
              <form onSubmit={handleApplySubmit} className="space-y-4">
                ...
                <div>
                  <label className="text-xs font-semibold text-base-content dark:text-white block mb-1">LinkedIn Profile or Portfolio URL *</label>
                  <input
                    type="url"
                    required
                ...
                <div>
                  <label className="text-xs font-semibold text-base-content dark:text-white block mb-1">Brief Note (Why Nowazone?)</label>
                  <textarea
                    rows={3}
